...
def showCommands():
    ...
    stringArgs +="\n[limit {Any Number} (Number of Images to be downloaded for each keyword)]"
...
def formPreviewedArguments():
    ...
    keywordsEntry.delete(0,END)                                         #Clear Everything in Keyword Entry Box
    keywordsEntry.insert(0,items[1:])                                   #Populate Entry Box with the Keywords that user has Wants from the Preview
    Records = {}
...
def downloadPreviewItems():
    ...
    if len(itemsToBeKept) == 0:                                         #User kept NO items, no download is required
        ...
        download()
#Delete Thumbnail and Preview Folder to neaten things
def clearPreviewFolder():
    shutil.rmtree(dir_path +"\downloads"+'\\'+previewQueue[previewQueueCounter])                #Delete Directory
    shutil.rmtree(dir_path +"\downloads"+'\\'+previewQueue[previewQueueCounter]+" - thumbnail") #Delete Thumbnails and directory
...
def previewActivity():
    ...
    def previewItemKeep():
        ...
        clearPreviewFolder()             #Clear Preview Directory
    ...
    if keyword == True:                                                     #Check if keywords Entry is not Empty
        ...
        if previewQueueCounter < len(previewQueue): #Check if there are still items to preview
            ...
            Records['limit'] = "8"                                          #Download 8 thumbnails for each Item in case some are duds
            download()
...
clearText.grid(row=3, column=3)
otherEntryText = Label(Menu,text = "Enter the arguments")                       #Other Arguments Entry Label
otherEntryText.grid(row=4, column=1)
otherEntryText = Label(Menu,text = "Example: limit 20 language czech")          #Other Arguments Hint
...
ClearButton.grid(row=4, column=3,columnspan=2)
